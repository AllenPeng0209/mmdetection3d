        ...
        for i in range(self._gt_num):
            ...
            if maximum > 0:
                ...
                if maximum >= thresh[index[0]] and self._gt_labels[index[0]] == self._dt_labels[index[1]]:
                    ...
                    self._iou[:, index[1]] = ma.masked
                    if self._tracking:
                        gtId = self._gtIds[index[0]]
                        dtId = self._dtIds[index[1]]
                        self._m[gtId] = dtId
...
class DeeprouteEval():
    def __init__(self, config_path):
        ...
        self._occlusion_thresh = occlusion_thresh
        self._cal_iou_bev = cal_iou_bev
        self._filter_points_number = filter_points_number
        self._num_features_for_pc = num_features_for_pc
        self._query_lists, self._query_distance_lists = self.get_query_list()

        self._eval_results = {label: {key: np.zeros(4) for key in self._query_lists.keys()} for label in self._interest_labels}
        self._eval_info = {key: {} for key in self._query_lists.keys()}
        self._eval_info_per_class = {label: {key: {} for key in self._query_lists.keys()} for label in self._interest_labels}
        self._num = len(self._query_lists)
        self._m = {}
        self.time = 0
        self._ignore_num = [np.array([0]*((len(distance_thresh)+1)*2)) for i in range(len(interest_labels))]

    def get_query_list(self):
        """
        The following two dictionaries can be obtained when self._distance_thresh = [20, 40, 80].
        query_lists = {'full_range': [[1], [0, 1, 2, 3], [0, 1]],
                     'within_80': [[1], [0, 1, 2], [0, 1]],
                     'within_40': [[1], [0, 1], [0, 1]],
                     'within_20': [[1], [0], [0, 1]],
                     'full_range_rm_occluded': [[1], [0, 1, 2, 3], [1]],
                     'within_80_rm_occluded': [[1], [0, 1, 2], [1]],
                     'within_40_rm_occluded': [[1], [0, 1], [1]],
                     'within_20_rm_occluded': [[1], [0], [1]]}

        query_distance_list = {'full_range': [[0,1], [0,1,2,3],[0,1]],
                     'within_80': [[0,1],[0,1,2],[0,1]],
                     'within_40': [[0,1],[0,1],[0,1]],
                     'within_20': [[0,1],[0],[0,1]],
                     'full_range_rm_occluded': [[0,1],[0,1,2,3],[1]],
                     'within_80_rm_occluded': [[0,1],[0,1,2],[1]],
                     'within_40_rm_occluded': [[0,1],[0,1],[1]],
                     'within_20_rm_occluded': [[0,1],[0],[1]]}
        """
        distance_thresh = self._distance_thresh
        namespace = []
    ...
    def get_eval(self, gt_loc, dt_loc, gt_boxes, dt_boxes, gt_names, dt_names, gtIds, dtIds, gt_occlusion, gt_ignore, idx, tracking):
        objstat = ObjStat(gt_loc,
                          dt_loc,
                          gt_boxes,
                          dt_boxes,
                          gt_names,
                          ...
                          gt_occlusion,
                          gt_ignore,
                          self._distance_thresh,
                          self._distance_compute_model,
                          self._occlusion_thresh,
                          gtIds,
                          dtIds,
                          self._name_to_label,
                          self._interest_labels,
                          self._thresh_map,
                          self._cal_iou_bev,
                          self._m,
                          tracking)
        ...
        for i in range(len(self._ignore_num)):
            self._ignore_num[i] = self._ignore_num[i] + objstat._ignore_num_per_frame[i]

        self._m = objstat.update_m
        self.time = self.time + objstat.time
        for key in self._eval_info.keys():
            missing = 0
            fp = 0
            missing_boxes = []
            fp_boxes = []
            for l in self._interest_labels:
                query_list = [[self._name_to_label.get(l)]] + self._query_lists.get(key)
                query_distance_list = [[self._name_to_label.get(l)]] + self._query_distance_lists.get(key)
                ...
                self._eval_results[l][key] += [label_missing, label_fp, label_gt, label_dt]
                missing += label_missing
                fp += label_fp

                if label_missing >0 or label_fp > 0:
                    self._eval_info_per_class[l][key][idx] = {'missing_boxes': [x.item() for x in objstat.query_idx(query_list, 1)],\
                                                              'fp_boxes': [x.item() for x in objstat.query_idx(query_list, 2)]}

            if missing > 0 or fp > 0:
                self._eval_info[key][idx] = {'missing_boxes': missing_boxes, 'fp_boxes': fp_boxes}
        #self.time += objstat.time

    def create_file(self, eval_path):
        for label in self._eval_info_per_class.keys():
            for key, value in self._eval_info_per_class[label].items():
                filename = Path(eval_path) / (label+"_"+key + '.pkl')
                with open(filename, 'wb') as fp:
                    pickle.dump(value, fp)

        for key, value in self._eval_info.items():
            filename = Path(eval_path)/("total_"+key+'.pkl')
            with open(filename, 'wb') as fp:
                pickle.dump(value, fp)
        self.generate_table(eval_path)

    def generate_table(self, eval_path):
        np.seterr(divide='ignore', invalid='ignore')
        total_data = np.zeros((len(self._distance_thresh)*2+2, 4))
        rows = []
        for l in self._eval_results.keys():
            rows = [k for k in self._eval_results[l].keys()]
            data = np.asarray([v for v in self._eval_results[l].values()]).reshape(len(self._distance_thresh)*2+2, 4)
            total_data += data
            self.draw_table(rows, eval_path, l, data, self._ignore_num[self._name_to_label[l.lower()]])

        total_ignore_num = self._ignore_num[0]
        for i in range(1, len(self._ignore_num)):
            total_ignore_num += self._ignore_num[i]
        self.draw_table(rows, eval_path, "total", total_data, total_ignore_num)

    def draw_table(self, rows, eval_path, l, data, ignore_num):
        data[:,2] = data[:, 2] - ignore_num
        tp_num = data[:, 2] - data[:, 0]
        data[:, 3] = tp_num + data[:, 1]
        missing_rate = np.round(1-data[:, 0]/data[:, 2], decimals=4).reshape(len(self._distance_thresh)*2+2, 1)
        fp_rate = np.round(1-data[:, 1]/data[:, 3], decimals=4).reshape(len(self._distance_thresh)*2+2, 1)
...
def compute_eval(config_path, eval_path, info_path, det_path, pcd_path, save_path, gt_occl_path, ignore_path, tracking, pool_num):

    eval = DeeprouteEval(config_path)
    name_to_label = eval._name_to_label
    interest_labels = eval._interest_labels
    occlusion_thresh = eval._occlusion_thresh
    filter_points_number = eval._filter_points_number
    ...
    for frame_id in tqdm(range(len(info_text_names))):
        ...
        # need to figure out how to get all the ground truth/detection unique ids
        gtIds = None
        dtIds = None
        eval.get_eval(gt_loc, dt_loc, gt_boxes, dt_boxes, gt_names, dt_names, gtIds, dtIds, gt_occlusion, gt_ignore, timeStamp, tracking)

    eval.create_file(eval_path)
    print("Evaluation Done!")


def main():
    root_path = "/home/jiamiaoxu/data/code/detection_evaluation/detection_Jiamiao/second/"
    info_path = os.path.join(root_path, "groundtruth")
    det_path = os.path.join(root_path, "evaluations/results")
    ...
    gt_occl_path = os.path.join(root_path, "kittiviewer/occlusion_detection.pkl")
    ignore_path = os.path.join(root_path, "kittiviewer/ignore_detection.pkl")
    #info_path = "/home/jiamiaoxu/data/code/detection_evaluation/detection_Jiamiao/second/cone_benchmark/label"
    #det_path = "/home/jiamiaoxu/data/code/detection_evaluation/detection_Jiamiao/second/evaluations/detections-c003"
    config_path = "/home/jiamiaoxu/data/code/detection_evaluation/perception_evaluation/configs/detection.eval.config"

    info_path = "/home/jiamiaoxu/data/code/prediction/sequence_det/second/cache/groundtruth"
    det_path = "/home/jiamiaoxu/data/code/prediction/sequence_det/second/cache/results/dr_pp_predict_branch_01_nr_split_rgb"
    pcd_path = "/home/jiamiaoxu/data/our_data/lidar/velodyne/20190412_rain_1"
    ...
    tracking = False
    t = time.time()
    compute_eval(config_path, eval_path, info_path, det_path, pcd_path, save_path, gt_occl_path, ignore_path, tracking, pool_num)
